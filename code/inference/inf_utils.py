...
def plot_traj(img, traj, color):
  ...
  traj = np.array(traj, dtype="float32") # dtype="float32"
  points = zip(traj[:-1], traj[1:])

  for p1, p2 in points:
    p_int1 = tuple(int(item) for item in p1)
    p_int2 = tuple(int(item) for item in p2)
    img = cv2.arrowedLine(img, p_int1, p_int2, color=color, thickness=1,
